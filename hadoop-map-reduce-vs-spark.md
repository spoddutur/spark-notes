...
Its not uncommon for a beginner to think Spark as a replacement to Hadoop or HDFS.. This blog is to understand what is Spark and its purpose

## Spark doesn't replace HDFS/Hadoop:
Spark doesn't replace HDFS. It is a very strong contender replacing MR computation engine on top of HDFS
#### HADOOP = HDFS + YARN + MR
...
2. Identify the disadvantages/hotspots in Map-Reduce - This gives an idea of the points that Spark has taken motivation from
3. How Spark works
```
...
## 2. Identify the disadvantages/hotspots in Map-Reduce and how Spark addressed it:
...
How do we get parallelism in Spark: by having different threads running inside the executor. Basically, in Spark, u have 1 executor JVM on each machine. Inside this executor JVM, u’ll have slots. In those slots, tasks run. 
